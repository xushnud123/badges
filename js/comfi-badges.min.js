const overlay = document.querySelector(".overlay"),
  ...
  dotsMapper = (_, $, t, e, C) => {
    ...
    return (
      _.forEach((C, s) => {
        a += `<div data-id=${s} data-step=${C} style="left:${
          (s + 1) * $
        }%" class="dots ${t === C && "dots-focus"} ${
          e && e - 1 > s && "dots-active"
        } ${t === _[_.length - 1] && "dots-active"}"></div>`;
      }),
      _.forEach((s, o) => {
        a += `<div
                      style="left: ${(o + 1) * $}%"
                      class="
                        content
                        ${t === s && "content-focus"} ${
          e && e - 1 > o && "content-active"
        } ${t === _[_.length - 1] && "content-active"}
                      ">
                      <div class="description_content"> Pay in ${s}</div>
                      <h1 class="price">
                        $ ${Math.round(Number(C) / Number(s))}
                      </h1>
                    </div>`;
      }),
      a
  ...
  track = (_, $ = [], t, e) =>
    `<div style="width:${
      _ === $[$.length - 1] ? 100 : t && t * e
    }%" class="track"></div>`,
  cardReturn = (_, $, t, e, C, a) =>
    a
    ...
    <button class="btn-close"><img class="close-img" src="https://firebasestorage.googleapis.com/v0/b/comfi-prod.appspot.com/o/Close.svg?alt=media&token=f28ea914-8ceb-427b-9c68-e1188112560d" alt="close btn"/></button>
        <div class="left-monthly">
          ...
          <a href="${a}" target="_blank"  rel="noopener noreferrer" class="monthly-a"><div class="monthly-btn"> Contact Sales</div></a>
      ...
      <div class="container-widget">
        ...
        <div class="row-slider">
          ...
          <div class="row-container">
            ...
            <div class="row-slider_bac">${track(t, _, e, $)}</div>
          </div>
        </div>
        <div class="rows">
...
function ComfiBadges({
  ...
  let s = document.getElementById(_),
    o,
    i,
    l = Math.round(100 / (C.length + 1));
  s &&
    (s.classList.add("comfi-badges"),
    s.classList.add(`style_${e}`),
    (s.innerHTML = `<div class="badges-wrapper">Pay only ${$} ${monthPrice(
      t,
      C
    )} today ${
      4 === e
      ...
      <div class="card">
        ${cardReturn(C, l, o, i, t, a)}
      </div>
    </div>
      `));
  let d = document.querySelectorAll(".wrapper-widget"),
    c = document.querySelectorAll(".wrapper-monthly"),
    r = document.querySelectorAll(".modal"),
    n = document.querySelectorAll(".badges-wrapper"),
    p = document.querySelectorAll(".btn-close"),
    h = document.querySelectorAll(".row-rows"),
    g = document.querySelectorAll(".row-slider_bac");
  d &&
    d.forEach((_, $) => {
    ...
    h &&
      h.forEach((_, $) => {
        _.addEventListener("click", (e) => {
          e.target.dataset.id &&
            ((i = Number(e.target.dataset.id) + 1),
            (o = Number(e.target.dataset.step)),
            (_.innerHTML = `${dotsMapper(C, l, o, i, t)}`),
            (Array.from(g)[$].innerHTML = `${track(o, C, i, l)}`));
        });
      }),
    r.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.remove("open-modal"),
          Array.from(r)[$].classList.add("hidden");
      });
    }),
    p &&
      p.forEach((_, $) => {
    ...
    n.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.add("open-modal"),
          Array.from(r)[$].classList.remove("hidden");
      });
    });
}
function ComfiBanner({ id: _ = "", style: $ = 1, discount: t = 10 }) {
  let e = document.getElementById(_);
  e &&
    (e.classList.add("comfi-banner"),
    e.classList.add(`style_${$}`),
    (e.innerHTML = `<p class='title'>Save ${t}% on annual plans and still pay monthly with</p>
        <span class='logo'>
        ${
          1 === $
            ? `<svg
