    ...
    ${
      s
        ...
        <div class="left-monthly">
          ...
          <h1 class="title_enjoy title-${l}">${styleComfiModal(r, i)}</h1>
          <h5 class="description_tag">No interest. No fees.</p>
          <a href="${o}" target="_blank"  rel="noopener noreferrer" class="monthly-a"><div class="monthly-btn"> Contact Sales</div></a>
      ...
      <div class="container-widget">
        ...
        ${
          ...
          <div class="row-container">
            <div class="row-rows slider-${l}">
              ${dotsMapper(_, $, e, t, a)}
              </div>
            <div class="row-slider_bac track-${l}">${track(e, _, t, $)}</div>
          </div>
        </div>`
            : ""
        }
...
function ComfiPopup({
  ...
  let s = document.getElementById(_),
    r = monthPrice($, e)?.[1],
    i = monthPrice($, e)?.[1],
    C = e.indexOf(monthPrice($, e)?.[1]) + 1,
    c = Math.round(100 / (e.length + 1));
  s &&
    (s.innerHTML = `
        <div class="card">
          ${cardReturn(e, c, i, C, $, t, _, !1, l, r)}
        </div>
        `);
  let d = document.querySelectorAll(".modal"),
    n = document.querySelectorAll(".badges-wrapper"),
    p = document.querySelectorAll(".btn-close"),
    m = document.querySelector(`.slider-${_}`),
    f = document.querySelector(`.track-${_}`),
    h = document.querySelector(`.title-${_}`);
  m &&
    m.addEventListener("click", (_) => {
      if (_.target.dataset.id) {
        (C = Number(_.target.dataset.id) + 1),
          (i = Number(_.target.dataset.step)),
          a && a(i),
          (r = i);
        let t = Array.from(
            document
              .createRange()
              .createContextualFragment(track(i, e, C, c))
              .querySelectorAll("*")
          ),
          o = Array.from(f.querySelectorAll("*"));
        t.forEach((_, $) => {
          let e = o[$];
          _.isEqualNode(e) ||
            "" === _.firstChild?.nodeValue.trim() ||
        ...
        let s = Array.from(
            document
              .createRange()
              .createContextualFragment(dotsMapper(e, c, i, C, $))
              .querySelectorAll("*")
          ),
          d = Array.from(m.querySelectorAll("*"));
        s.forEach((_, $) => {
          let e = d[$];
          _.isEqualNode(e) ||
            "" === _.firstChild?.nodeValue.trim() ||
          ...
          (h.innerHTML = styleComfiModal(l, r));
      }
    }),
    d.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.remove("open-modal"),
          Array.from(d)[$].classList.remove("hidden");
      });
    }),
    p &&
      p.forEach((_, $) => {
    ...
    n.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.add("open-modal"),
          Array.from(d)[$].classList.add("hidden");
      });
    });
}
