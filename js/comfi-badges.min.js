const overlay = document.querySelector(".overlay"),
  ...
  priceCalculate = (_, $) => Math.round(Number(_) / Number($)),
  dotsMapper = (_, $, e, t, a) => {
    let o = "";
    return (
      _.forEach((a, l) => {
        o += `<div data-id=${l} data-step=${a} style="left:${percentageHandler(
          l,
          _,
          $
        )}%" class="dots ${e === a && "dots-focus"} ${
          t && t - 1 > l && "dots-active"
        } "></div>`;
      }),
      _.forEach((l, s) => {
        o += `<div
                      style="left: ${percentageHandler(s, _, $)}%"
                      class="
                        content
                        ${e === l && "content-focus"} ${
          t && t - 1 > s && "content-active"
        } ${e === _[_.length - 1] && "content-active"}
                      ">
                      <div class="description_content"> Pay in ${l}</div>
                      <h1 class="price">
                        $ ${priceCalculate(a, l)}
                      </h1>
                    </div>`;
  ...
  track = (_, $ = [], e, t) =>
    `<div style="width:${
      _ === $[$.length - 1] ? 100 : percentageHandler(e - 1, $, t)
    }%" class="track"></div>`,
  planStates = (_, $, e) =>
    1 === _.length
      ? `<div class="left-widget-plans">
    <p class="left-widget-plans_p">pay in ${_[0]}</p>
    <h2 class="left-widget-plans_h2"> ${priceCalculate($, _[0])}/mo</h2>
    <h3 class="left-widget-currency">${e}</h3>
  </div>
  <img
  ...
  styleComfiModal = (_, $) =>
    2 === _
      ? `Split your payment into ${$} interest free payments`
      : "Enjoy annual discounts now, pay monthly",
  cardReturn = (_, $, e, t, a, o, l, s, r, i, C) =>
    l
    ...
    ${
      r
        ...
        <div class="left-monthly">
          ...
          <h1 class="title_enjoy title-${s}">${styleComfiModal(i, C)}</h1>
          <h5 class="description_tag">No interest. No fees.</p>
          <a href="${l}" target="_blank"  rel="noopener noreferrer" class="monthly-a"><div class="monthly-btn"> Contact Sales</div></a>
      ...
      <div class="container-widget">
        ...
        ${
          ...
          <div class="row-container">
            <div class="row-rows slider-${s}">
              ${dotsMapper(_, e, t, a, o)}
              </div>
            <div class="row-slider_bac track-${s}">${track(t, _, a, e)}</div>
          </div>
        </div>`
            : ""
        }
...
function ComfiBadges({
  ...
  let r = document.getElementById(_),
    i = monthPrice(e, a)?.[1],
    C = monthPrice(e, a)?.[1],
    c = a.indexOf(monthPrice(e, a)?.[1]) + 1,
    d = Math.round(100 / (a.length + 1));
  r &&
    (r.classList.add("comfi-badges"),
    r.classList.add(`style_${t}`),
    (r.innerHTML = `<div class="badges-wrapper">Pay only ${$} ${
      monthPrice(e, a)?.[0]
    } today ${
      4 === t
        ...
        <div class="modal">
      <div class="card">
        ${cardReturn(a, $, d, C, c, e, o, _, !0, s, i)}
      </div>
    </div>
      `));
  let n = document.querySelectorAll(".wrapper-widget"),
    p = document.querySelectorAll(".wrapper-monthly"),
    m = document.querySelectorAll(".modal"),
    f = document.querySelectorAll(".badges-wrapper"),
    h = document.querySelectorAll(".btn-close"),
    g = document.querySelector(`.slider-${_}`),
    v = document.querySelector(`.track-${_}`),
    u = document.querySelector(`.title-${_}`);
  n &&
    n.forEach((_, $) => {
      _.addEventListener("click", (_) => _.stopPropagation());
    }),
    p &&
      p.forEach((_) => {
        _.addEventListener("click", (_) => _.stopPropagation());
    ...
    g &&
      g.addEventListener("click", (_) => {
        if (_.target.dataset.id) {
          (c = Number(_.target.dataset.id) + 1),
            (i = C = Number(_.target.dataset.step)),
            l && l(C);
          let $ = Array.from(
              document
                .createRange()
                .createContextualFragment(track(C, a, c, d))
                .querySelectorAll("*")
            ),
          ...
          $.forEach((_, $) => {
            let e = t[$];
            _.isEqualNode(e) ||
              "" === _.firstChild?.nodeValue.trim() ||
              (e.textContent = _.textContent),
          ...
          let o = Array.from(
              document
                .createRange()
                .createContextualFragment(dotsMapper(a, d, C, c, e))
                .querySelectorAll("*")
            ),
            r = Array.from(g.querySelectorAll("*"));
          o.forEach((_, $) => {
            let e = r[$];
            _.isEqualNode(e) ||
              "" === _.firstChild?.nodeValue.trim() ||
            ...
            (u.innerHTML = styleComfiModal(s, i));
        }
      }),
    m.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.remove("open-modal"),
          Array.from(m)[$].classList.remove("hidden");
      });
    }),
    h &&
      h.forEach((_, $) => {
    ...
    f.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.add("open-modal"),
          Array.from(m)[$].classList.add("hidden");
      });
    });
}
function CustomHandler() {
  let _ = document.querySelectorAll(".wrapper-widget"),
    $ = document.querySelectorAll(".wrapper-monthly"),
    e = document.querySelectorAll(".modal"),
    t = document.querySelectorAll(".badges-wrapper"),
    a = document.querySelectorAll(".btn-close"),
    o = document.querySelector(`.slider-${id}`),
    l = document.querySelector(`.track-${id}`);
  _ &&
    _.forEach((_, $) => {
      _.addEventListener("click", (_) => _.stopPropagation());
    }),
    ...
    o &&
      o.addEventListener("click", (_) => {
        if (_.target.dataset.id) {
          (index = Number(_.target.dataset.id) + 1),
            (step = Number(_.target.dataset.step)),
          ...
          $.forEach((_, $) => {
            ...
            _.isEqualNode(t) ||
              "" === _.firstChild?.nodeValue.trim() ||
              (t.textContent = _.textContent),
              Array.from(_.attributes).forEach((_) =>
                t.setAttribute(_.name, _.value)
              );
          });
          let t = Array.from(
              document
            ...
            a = Array.from(o.querySelectorAll("*"));
          t.forEach((_, $) => {
            let e = a[$];
            _.isEqualNode(e) ||
              "" === _.firstChild?.nodeValue.trim() ||
    ...
    e.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.remove("open-modal"),
          Array.from(e)[$].classList.remove("hidden");
      });
    }),
    a &&
      a.forEach((_, $) => {
    ...
    t.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.add("open-modal"),
          Array.from(e)[$].classList.add("hidden");
      });
    });
}
function ComfiPopup({
  ...
  let r = document.getElementById(_),
    i = monthPrice(e, t)?.[1],
    C = monthPrice(e, t)?.[1],
    c = t.indexOf(monthPrice(e, t)?.[1]) + 1,
    d = Math.round(100 / (t.length + 1));
  r &&
    (r.innerHTML = `
        <div class="card">
          ${cardReturn(t, $, d, C, c, e, a, _, !1, s, i)}
        </div>
        `);
  let n = document.querySelectorAll(".modal"),
    p = document.querySelectorAll(".badges-wrapper"),
    m = document.querySelectorAll(".btn-close"),
    f = document.querySelector(`.slider-${_}`),
    h = document.querySelector(`.track-${_}`),
    g = document.querySelector(`.title-${_}`);
  f &&
    f.addEventListener("click", (_) => {
      if (_.target.dataset.id) {
        (c = Number(_.target.dataset.id) + 1),
          (C = Number(_.target.dataset.step)),
          o && o(C),
          (i = C);
        let $ = Array.from(
            document
              .createRange()
              .createContextualFragment(track(C, t, c, d))
              .querySelectorAll("*")
          ),
          a = Array.from(h.querySelectorAll("*"));
        $.forEach((_, $) => {
          let e = a[$];
          _.isEqualNode(e) ||
            "" === _.firstChild?.nodeValue.trim() ||
        ...
        let l = Array.from(
            document
              .createRange()
              .createContextualFragment(dotsMapper(t, d, C, c, e))
              .querySelectorAll("*")
          ),
          r = Array.from(f.querySelectorAll("*"));
        l.forEach((_, $) => {
          let e = r[$];
          _.isEqualNode(e) ||
            "" === _.firstChild?.nodeValue.trim() ||
          ...
          (g.innerHTML = styleComfiModal(s, i));
      }
    }),
    n.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.remove("open-modal"),
          Array.from(n)[$].classList.remove("hidden");
      });
    }),
    m &&
      m.forEach((_, $) => {
    ...
    p.forEach((_, $) => {
      _.addEventListener("click", () => {
        document.body.classList.add("open-modal"),
          Array.from(n)[$].classList.add("hidden");
      });
    });
}
function ComfiBanner({ id: _ = "", style: $ = 1, discount: e = 10 }) {
  let t = document.getElementById(_);
  t &&
    (t.classList.add("comfi-banner"),
    t.classList.add(`style_${$}`),
    (t.innerHTML = `<p class='title'>Save ${e}% on annual plans and still pay monthly with</p>
        <span class='logo'>
        ${
          1 === $
            ? `<svg
